...
namespace CRS_PRE
{
    ...
    /*      Opción: Informe R04 - Reporte View                            */
    /*       Autor: JEJR - Crearsis             Fecha: 01-07-2022         */
    /**********************************************************************/
    public partial class adp002_R04w : Form
    {
        ...
        public string vp_rut_ini;
        public string vp_rut_fin;
        ...
        private void frm_Load(object sender, EventArgs e)
        {
            ...
            // Obtiene la descripcion de la ruta inicial
            Tabla = new DataTable();
            Tabla = o_adp007.Fe_con_rut(int.Parse(vp_rut_ini));
            if (Tabla.Rows.Count > 0)
            {
                vp_rut_ini = Tabla.Rows[0]["va_ide_rut"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_rut"].ToString();
            }

            // Obtiene la descripcion de la ruta final
            Tabla = new DataTable();
            Tabla = o_adp007.Fe_con_rut(int.Parse(vp_rut_fin));
            if (Tabla.Rows.Count > 0)
            {
                vp_rut_fin = Tabla.Rows[0]["va_ide_rut"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_rut"].ToString();
            }
            ...
            adp002_R04.SetDatabaseLogon(o_ads007.va_ide_usr, o_ads007.va_pas_usr, o_ads007.va_ser_bda + "\\" + o_ads007.va_ins_bda, o_ads007.va_nom_bda);
            // Paso los datos obtenidos del procedimiento en la anterior ventana
            adp002_R04.SetDataSource(frm_dat);
            // Para enviar parametros directos al reporte (nombre del parametro en crystal report, valor que se enviara)
            adp002_R04.SetParameterValue("vc_nom_emp", va_nom_emp);
            adp002_R04.SetParameterValue("vc_gru_ini", vp_gru_ini);
            adp002_R04.SetParameterValue("vc_gru_fin", vp_gru_fin);
            adp002_R04.SetParameterValue("vc_tip_ini", va_tip_ini);
            adp002_R04.SetParameterValue("vc_tip_fin", va_tip_fin);
            adp002_R04.SetParameterValue("vc_atr_ini", vp_atr_ini);
            adp002_R04.SetParameterValue("vc_atr_fin", vp_atr_fin);
            adp002_R04.SetParameterValue("vc_rut_ini", vp_rut_ini);
            adp002_R04.SetParameterValue("vc_rut_fin", vp_rut_fin);
            adp002_R04.SetParameterValue("vc_est_ado", vp_est_ado);
            adp002_R04.SetParameterValue("vc_ide_usr", o_ads007.va_ide_usr);
        }
