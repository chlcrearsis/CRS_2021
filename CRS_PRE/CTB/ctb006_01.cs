...
namespace CRS_PRE
{
    public partial class ctb006_01 : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0)
        {
            //Limpia Grilla
            dg_res_ult.Rows.Clear();

            
            tabla = o_ctb006.Fe_bus_car(ar_tex_bus, ar_prm_bus);
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                {
                    dg_res_ult.Rows.Add();
                    dg_res_ult.Rows[i].Cells["va_cod_ley"].Value = tabla.Rows[i]["va_cod_ley"].ToString();
                    dg_res_ult.Rows[i].Cells["va_nom_ley"].Value = tabla.Rows[i]["va_nom_ley"].ToString();
                  
                }
                tb_sel_bus.Text = tabla.Rows[0]["va_cod_ley"].ToString();
                lb_des_bus.Text = tabla.Rows[0]["va_nom_ley"].ToString();
            }
        ...
        private void fi_con_sel()
        {
            ...
            tabla = o_ctb006.Fe_con_ley(tb_sel_bus.Text);
            if (tabla.Rows.Count == 0)
            {
                lb_des_bus.Text = "** NO existe";
                return;
            ...
            lb_des_bus.Text = Convert.ToString(tabla.Rows[0]["va_nom_ley"].ToString());
        }
        /// <summary>
        /// - > Función que selecciona la fila en el Datagrid que La Leyenda Modificó
        /// </summary>
        private void fi_sel_fil(int cod_ley)
        {

            fi_bus_car(tb_tex_bus.Text, cb_prm_bus.SelectedIndex);

            if (cod_ley.ToString() != null)
            {
                try
                {
                    for (int i = 0; i < dg_res_ult.Rows.Count; i++)
                    {
                        if (dg_res_ult.Rows[i].Cells[0].Value.ToString() == cod_ley.ToString())
                        {
        ...
        {
            string res_fun = "";
            tab_dat = o_ctb006.Fe_con_ley(sel_ecc);
            if (tab_dat.Rows.Count == 0)
            {
                res_fun = "La Leyenda que desea editar, no se encuentra registrada";
            }

            if (res_fun != "")
            {
                MessageBox.Show(res_fun, "Edita Leyenda", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        public bool fi_ver_hds(string sel_ecc)
        {
            ...
            if (tab_dat.Rows.Count == 0)
            {
                MessageBox.Show("La Leyenda ya no se encuentra registrada en la base de datos.", "Consulta Leyenda", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        /// </summary>
        public void Fe_ley_frm(int cod_tus)
        {
