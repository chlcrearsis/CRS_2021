...
namespace CRS_PRE
{
    public partial class cmr016_01 : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0)
        {
            ...
            tabla = o_cmr016.Fe_bus_car(ar_tex_bus, ar_prm_bus);
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                {
                    dg_res_ult.Rows.Add();
                    dg_res_ult.Rows[i].Cells["va_cod_act"].Value = tabla.Rows[i]["va_cod_act"].ToString();
                    dg_res_ult.Rows[i].Cells["va_nom_act"].Value = tabla.Rows[i]["va_nom_act"].ToString();
                  
                }
                tb_sel_bus.Text = tabla.Rows[0]["va_cod_act"].ToString();
                lb_des_bus.Text = tabla.Rows[0]["va_nom_act"].ToString();
            }
        ...
        private void fi_con_sel()
        {
            ...
            tabla = o_cmr016.Fe_con_act(tb_sel_bus.Text);
            if (tabla.Rows.Count == 0)
            {
                lb_des_bus.Text = "** NO existe";
                return;
            ...
            lb_des_bus.Text = Convert.ToString(tabla.Rows[0]["va_nom_act"].ToString());
        }
        /// <summary>
        /// - > Función que selecciona la fila en el Datagrid que La actividad economica Modificó
        /// </summary>
        ...
        {
            string res_fun = "";
            tab_dat = o_cmr016.Fe_con_act(sel_ecc);
            if (tab_dat.Rows.Count == 0)
            {
                res_fun = "La actividad economica que desea editar, no se encuentra registrada";
            }

            if (res_fun != "")
            {
                MessageBox.Show(res_fun, "Edita Actividad economica", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        public bool fi_ver_hds(string sel_ecc)
        {
            ...
            if (tab_dat.Rows.Count == 0)
            {
                MessageBox.Show("La actividad economica ya no se encuentra registrada en la base de datos.", "Consulta Actividad economica", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
