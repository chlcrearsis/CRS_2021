...
{
    public partial class cmr005_01 : Form
    {
        ...
        private void fi_bus_car(  )
        {
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                tb_sel_ide.Text = dg_res_ult.Rows[dg_res_ult.CurrentCell.RowIndex].Cells["va_ide_vta"].Value.ToString();
        ...
        private void fi_con_sel()
        {
            ...
            tabla = new DataTable();
            tabla = o_cmr005.Fe_con_vta(tb_sel_ide.Text,int.Parse(tb_sel_ano.Text));
        ...
        private void fi_sel_fil( string ide_vta, int ges_tio)
        {
            ...
            if (ide_vta != null || ges_tio.ToString() != null)
            {
                try
                {
                    for (int i = 0; i < dg_res_ult.Rows.Count; i++)
                    {
                        if (dg_res_ult.Rows[i].Cells["va_ide_vta"].Value.ToString().ToUpper() == ide_vta.ToString() &&
        ...
        public bool fi_ver_edi()
        {
            ...
            if (val == 0)
                res_fun = "La Venta no se encuentra registrado";


            if (res_fun != "")
            {
                MessageBox.Show(res_fun, "Edita Venta", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_ano.Focus();
                return false;
            }
            ...
            tab_dat = o_cmr005.Fe_con_vta(tb_sel_ide.Text,  int.Parse(tb_sel_ano.Text));
            if (tab_dat.Rows.Count == 0)
            {
                res_fun = "La Venta que desea editar, no se encuentra registrado";
            }
        ...
        public bool fi_ver_con()
        {
            ...
            if (tb_sel_ide.Text.Trim() == "")
                res_fun = "La Venta, no se encuentra registrada";
            ...
            tab_dat = o_cmr005.Fe_con_exi_vta(tb_sel_ide.Text, int.Parse(tb_sel_ano.Text));
            if (tab_dat.Rows.Count == 0)
            {
                MessageBox.Show("La Venta ya no se encuentra registrada en la base de datos.", "Consulta Venta", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_ide.Focus();
                return false;
            }

            //tab_dat = new DataTable();
            //tab_dat = o_cmr005.Fe_cmr005_05a_p01(tb_sel_tal.Text, int.Parse(tb_sel_ano.Text));
        ...
        private void Mn_con_cmp_Click(object sender, EventArgs e)
        {
            ...
            DataTable frm_dat = new DataTable();
            frm_dat.Columns.Add("va_ide_doc");
            frm_dat.Columns.Add("va_ges_doc");

            frm_dat.Rows.Add();
            frm_dat.Rows[0]["va_ide_doc"] = tb_sel_ide.Text;
            frm_dat.Rows[0]["va_ges_doc"] = tb_sel_ano.Text;


            /* ABRE FORMULARIO CONSULTA */
            cmr005_05w frm = new cmr005_05w();
            cl_glo_frm.abrir(this, frm, cl_glo_frm.ventana.nada, cl_glo_frm.ctr_btn.no, frm_dat);
        ...
        private void Fi_for_mat()
        {
            ...
            string doc_vta =  tb_sel_ide.Text.Substring(0,3);
            ...
            if (doc_vta == "VTF") // Venta con factura
            {
                switch (nro_for)
                {
                    case 0:
        ...
        private void Mn_con_avi_Click(object sender, EventArgs e)
        {
            ...
            //cmr005_05bw frm = new cmr005_05bw();
            //cl_glo_frm.abrir(this, frm, cl_glo_frm.ventana.nada, cl_glo_frm.ctr_btn.no, tab_dat);
        ...
        private void Mn_lis_vta_Click(object sender, EventArgs e)
        {
            cmr005_R01p frm = new cmr005_R01p();
