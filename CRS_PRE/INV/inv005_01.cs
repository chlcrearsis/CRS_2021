...
{
    public partial class inv005_01 : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0, string ar_est_bus = "T")
        {
            ...
            tabla = o_inv005.Fe_bus_car(ar_tex_bus, ar_prm_bus, ar_est_bus);
            ...
            if (tabla.Rows.Count > 0)
            {
                for (int i = 0; i < tabla.Rows.Count; i++)
                {
                    ...
                    dg_res_ult.Rows[i].Cells["va_nom_umd"].Value = tabla.Rows[i]["va_nom_umd"].ToString();
                    
                    
                }
                tb_sel_bus.Text = tabla.Rows[0]["va_cod_umd"].ToString();
                lb_des_bus.Text = tabla.Rows[0]["va_nom_umd"].ToString();
            }
        ...
        private void fi_con_sel()
        {
            ...
            tabla = o_inv005.Fe_con_umd(tb_sel_bus.Text);
            if (tabla.Rows.Count == 0)
            {
                lb_des_bus.Text = "** NO existe";
                return;
            ...
            lb_des_bus.Text = Convert.ToString(tabla.Rows[0]["va_nom_umd"].ToString());
        }
        /// <summary>
        /// - > Función que selecciona la fila en el Datagrid que La Unidad Modificó
        /// </summary>
        private void fi_sel_fil(string cod_umd)
        {
         

            fi_bus_car(tb_tex_bus.Text, 1, "T");
            ...
            if (cod_umd != null)
            {
                try
                {
                    ...
                    {
                        if (dg_res_ult.Rows[i].Cells[0].Value.ToString().ToUpper() == cod_umd.ToUpper())
                        {
        ...
        {
            string res_fun = "";
            tab_dat = o_inv005.Fe_con_umd(sel_ecc);
            if (tabla.Rows.Count == 0)
            {
                res_fun = "La Unidad que desea editar, no se encuentra registrada";
            }

            if (res_fun != "")
            {
                MessageBox.Show(res_fun, "Edita Unidad", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        public bool fi_ver_hds(string sel_ecc)
        {
            ...
            {
                MessageBox.Show("La Unidad ya no se encuentra registrada en la base de datos.", "Consulta Unidad", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        /// </summary>
        public void Fe_act_frm(string cod_umd)
        {
         

            fi_bus_car(tb_tex_bus.Text, 1, "T");

            if (cod_umd.ToString() != null)
            {
                try
                {
                    for (int i = 0; i < dg_res_ult.Rows.Count; i++)
                    {
                        if (dg_res_ult.Rows[i].Cells[0].Value.ToString().ToUpper() == cod_umd.ToString().ToUpper())
                        {
                            dg_res_ult.Rows[i].Selected = true;
                            dg_res_ult.FirstDisplayedScrollingRowIndex = i;
        ...
        private void Mn_list_umd_Click(object sender, EventArgs e)
        {
            //inv005_R01p frm = new inv005_R01p();
            //cl_glo_frm.abrir(this, frm, cl_glo_frm.ventana.nada, cl_glo_frm.ctr_btn.si);
        }
