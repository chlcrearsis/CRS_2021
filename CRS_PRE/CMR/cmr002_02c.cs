...
namespace CRS_PRE
{
    public partial class cmr002_02c : Form
    {
        ...
        {
            tb_nro_lis_ini.Text = "0";// frm_dat.Rows[0]["va_cod_lis"].ToString();
            lb_nom_lis_ini.Text = "0";// frm_dat.Rows[0]["va_nom_lis"].ToString();

            tb_nro_lis_fin.Text = "0"; // frm_dat.Rows[0]["va_cod_lis"].ToString();
            lb_nom_lis_fin.Text = "0"; // frm_dat.Rows[0]["va_nom_lis"].ToString();
        ...
        string Fi_val_dat()
        {
            // verifica que haya digitado un producto
            if(tb_cod_pro.Text == "")
                return "Debe de seleccionar un producto";
            if(dg_res_ult.Rows.Count == 0)
                return "Debe de definir precio para el producto al menos en una lista de precio";
        ...
        private void Bt_ace_pta_Click(object sender, EventArgs e)
        {
            ...
            if (msg_val != "")
            {
                ...
                msg_res = MessageBox.Show("Esta seguro de grabar la informacion ?", "Crea Precio", MessageBoxButtons.OKCancel);
                ...
                if (msg_res == DialogResult.OK)
                {
                    ...
                    {
                        // obtiene datos
                        int nro_lis = int.Parse(dg_res_ult.Rows[i].Cells["va_cod_lis"].Value.ToString());
                        decimal pre_cio = decimal.Parse(dg_res_ult.Rows[i].Cells["va_pre_cio"].Value.ToString());
                        decimal pmx_des = decimal.Parse(dg_res_ult.Rows[i].Cells["va_pmx_des"].Value.ToString());
                    ...
                    MessageBox.Show("Los Precios se grabaron correctamente", "Define precio", MessageBoxButtons.OK);
                    Fi_lim_pia();
                    cl_glo_frm.Cerrar(this);
                }
        ...
        private void dg_res_ult_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                // VALIDA  Y FORMATEA PRECIO
                int val_row = e.RowIndex;

                val_pre = decimal.Parse(dg_res_ult.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString());
        ...
        private void dg_res_ult_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                nro_row = e.RowIndex;
                nro_cel = e.ColumnIndex;
                if (dg_res_ult.Rows[e.RowIndex].Cells[e.ColumnIndex].Value == null)
                    val_cel = 0m;
                else
                    val_cel = decimal.Parse(dg_res_ult.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString());

                int nro_dec = int.Parse(dg_res_ult.Rows[e.RowIndex].Cells["va_nro_dec"].Value.ToString());

                // Formatea valor
                dg_res_ult.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = Fi_for_dec(val_cel, nro_dec).ToString();
            ...
            {
                MessageBox.Show(ex.Message,"Error",MessageBoxButtons.OK,MessageBoxIcon.Error);
                dg_res_ult.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = val_pre.ToString();
