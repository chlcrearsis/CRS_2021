...
{
    public partial class cmr014_01d : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0, string ar_est_bus = "H")
        {
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                {
                    dg_res_ult.Rows.Add();
                    dg_res_ult.Rows[i].Cells["va_cod_cob"].Value = tabla.Rows[i]["va_cod_ide"].ToString();
                    dg_res_ult.Rows[i].Cells["va_nom_bre"].Value = tabla.Rows[i]["va_nom_bre"].ToString();
        ...
        public bool fi_ver_edi(int sel_ecc)
        {
            ...
            if (tabla.Rows.Count == 0)
            {
                res_fun = "El cobrador que desea editar, no se encuentra registrado";
            }

            if (res_fun != "")
            {
                MessageBox.Show(res_fun, "Edita cobrador", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
        ...
        public bool fi_ver_hds(int sel_ecc)
        {
            ...
            {
                MessageBox.Show("EL cobrador ya no se encuentra registrado en la base de datos.", "Consulta cobrador", MessageBoxButtons.OK, MessageBoxIcon.Error);
                tb_sel_bus.Focus();
                return false;
            }
