...
namespace CRS_PRE
{
    public partial class cmr014_04b : Form
    {
        ...
        protected string Fi_val_dat()
        {
            ...
            {
                tb_cod_ven.Focus();
                return "el cobrador que desea crear ya NO se encuentra registrada";
            }
            if (tb_nom_ven.Text.Trim() == "")
            {
                tb_nom_ven.Focus();
                return "Debe proporcionar el Nombre para el cobrador";
            }
        ...
        private void Bt_ace_pta_Click(object sender, EventArgs e)
        {
            ...
            if (frm_dat.Rows[0]["va_est_ado"].ToString() == "H")
            {
                msg_res = MessageBox.Show("Esta seguro de Deshabilitar el cobrador?", "Deshabilita cobrador", MessageBoxButtons.OKCancel);
                if (msg_res == DialogResult.OK)
                {
                    //Deshabilita cobrador
                    o_cmr014.Fe_des_hab(int.Parse(tb_cod_ven.Text),2);
                }
            }
            else
            {
                msg_res = MessageBox.Show("Esta seguro de Habilitar el cobrador?", "Habilita cobrador", MessageBoxButtons.OKCancel);
                if (msg_res == DialogResult.OK)
                {
                    //Habilita cobrador
            ...
            //MessageBox.Show("Los datos se grabaron correctamente", "cobrador de Precio", MessageBoxButtons.OK);
