...
namespace CRS_PRE
{
    public partial class ads008_06 : Form
    {
        ...
        private void Bt_ace_pta_Click(object sender, EventArgs e)
        {
            ...
            msg_res = MessageBox.Show("Esta seguro de editar la informacion?", "Permiso sobre Lista de precios", MessageBoxButtons.OKCancel,MessageBoxIcon.Question);
            if (msg_res == DialogResult.OK)
            {
                for (int i = 0; i < dg_res_ult.RowCount ; i++)
                {
                    bool chk_val = (bool)dg_res_ult.Rows[i].Cells["va_per_mis"].Value;
                    string ide_apl= tab_cmr001.Rows[i]["va_cod_lis"].ToString();

                    if (chk_val == true)
                    { 
                        o_ads008.Fe_ads008_04(tb_ide_usr.Text,"cmr001", ide_apl);
                        o_ads008.Fe_ads008_03(tb_ide_usr.Text, "cmr001", ide_apl);
                    }
                    if (chk_val == false)
                        o_ads008.Fe_ads008_04(tb_ide_usr.Text, "cmr001", ide_apl);
                }
        ...
        {
            if (tab_cmr001.Rows[e.RowIndex]["va_est_ado"].ToString() == "V")
