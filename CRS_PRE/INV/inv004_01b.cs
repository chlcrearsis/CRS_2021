...
{
    public partial class inv004_01b : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0, string ar_est_bus = "T", string ar_cod_fam = "000000")
        {
            ...
            tabla = o_inv004.Fe_bus_car(ar_tex_bus, ar_prm_bus, est_bus, int.Parse(frm_pad.tb_cod_lis.Text), int.Parse(frm_pad.tb_cod_bod.Text), ar_cod_fam);
            ...
            if (tabla.Rows.Count > 0)
            {
                for (int i = 0; i < tabla.Rows.Count; i++)
                {
                    ...
                    dg_res_ult.Rows[i].Cells["va_sal_can"].Value = tabla.Rows[i]["va_sal_can"].ToString();
                    dg_res_ult.Rows[i].Cells["va_und_vta"].Value = tabla.Rows[i]["va_und_vta"].ToString();
                    dg_res_ult.Rows[i].Cells["va_sal_vta"].Value = tabla.Rows[i]["va_sal_vta"].ToString();
                    dg_res_ult.Rows[i].Cells["va_pre_cio"].Value = tabla.Rows[i]["va_pre_vta"].ToString();
        ...
        private void bt_con_exi_Click(object sender, EventArgs e)
        {
            // Verifica concurrencia de datos para habilitar/deshabilitar
            if (fi_ver_con(tb_sel_ecc.Text) == false)
                return;
