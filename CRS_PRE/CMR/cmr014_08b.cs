...
namespace CRS_PRE
{
    public partial class cmr014_08b : Form
    {
        ...
        protected string Fi_val_dat(){
            ...
            if (Tabla.Rows.Count == 0){
                return "El Cobrador NO esta definido en el Sistema";
            }
        ...
        {            
            ...
            try
            {
                ...
                msg_res = MessageBox.Show("Esta seguro de editar la informacion?", "Permiso Usuario p/Cobrador", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                if (msg_res == DialogResult.OK)
                {
                    for (int i = 0; i < dg_res_ult.RowCount; i++)
                    {
                        ...
                        // Elimina el permiso
                        o_ads008.Fe_eli_min(ide_usr, "cmr014", "2", tb_cod_cob.Text);

                        // Habilita Permiso s/Usuario SI esta autorizado
                        if (chk_val == true)                        
                            o_ads008.Fe_nue_reg(ide_usr, "cmr014", "2", tb_cod_cob.Text);                        
                    }
                    cl_glo_frm.Cerrar(this);
                }
            }catch (Exception ex){
                MessageBox.Show(ex.Message, "Permiso Usuario p/Cobrador", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
