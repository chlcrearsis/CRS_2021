...
namespace CRS_PRE
{
    ...
    /**********************************************************************/
    public partial class adp002_R03w : Form
    {
        ...
        public string vp_tip_at1;
        public string vp_atr_in1;
        public string vp_atr_fi1;
        public string vp_tip_at2;
        public string vp_atr_in2;
        public string vp_atr_fi2;
        ...
        private void frm_Load(object sender, EventArgs e)
        {
            ...
            // Obtiene la descripcion del Tipo de Atributo inicial y final criterio 1
            string va_tip_in1 = vp_tip_at1;
            string va_tip_fi1 = vp_tip_at1;
            Tabla = new DataTable();
            Tabla = o_adp003.Fe_con_tip(int.Parse(vp_tip_at1));
            if (Tabla.Rows.Count > 0)
            {
                va_tip_in1 = Tabla.Rows[0]["va_nom_tip"].ToString() + " Inicial : ";
                va_tip_fi1 = Tabla.Rows[0]["va_nom_tip"].ToString() + " Final : ";
            }

            // Obtiene la descripcion del atributo inicial criterio 1
            Tabla = new DataTable();
            Tabla = o_adp004.Fe_con_atr(int.Parse(vp_tip_at1), int.Parse(vp_atr_in1));
            if (Tabla.Rows.Count > 0)
            {
                vp_atr_in1 = Tabla.Rows[0]["va_ide_atr"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_atr"].ToString();
            }

            // Obtiene la descripcion del atributo final creterio 1
            Tabla = new DataTable();
            Tabla = o_adp004.Fe_con_atr(int.Parse(vp_tip_at1), int.Parse(vp_atr_fi1));
            if (Tabla.Rows.Count > 0)
            {
                vp_atr_fi1 = Tabla.Rows[0]["va_ide_atr"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_atr"].ToString();
            }

            // Obtiene la descripcion del Tipo de Atributo inicial y final criterio 2
            string va_tip_in2 = vp_tip_at2;
            string va_tip_fi2 = vp_tip_at2;
            Tabla = new DataTable();
            Tabla = o_adp003.Fe_con_tip(int.Parse(vp_tip_at2));
            if (Tabla.Rows.Count > 0)
            {
                va_tip_in2 = Tabla.Rows[0]["va_nom_tip"].ToString() + " Inicial : ";
                va_tip_fi2 = Tabla.Rows[0]["va_nom_tip"].ToString() + " Final : ";
            }

            // Obtiene la descripcion del atributo inicial criterio 2
            Tabla = new DataTable();
            Tabla = o_adp004.Fe_con_atr(int.Parse(vp_tip_at2), int.Parse(vp_atr_in2));
            if (Tabla.Rows.Count > 0)
            {
                vp_atr_in2 = Tabla.Rows[0]["va_ide_atr"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_atr"].ToString();
            }

            // Obtiene la descripcion del atributo final creterio 2
            Tabla = new DataTable();
            Tabla = o_adp004.Fe_con_atr(int.Parse(vp_tip_at2), int.Parse(vp_atr_fi2));
            if (Tabla.Rows.Count > 0)
            {
                vp_atr_fi2 = Tabla.Rows[0]["va_ide_atr"].ToString() + "  -  " +
                             Tabla.Rows[0]["va_nom_atr"].ToString();
            }
            ...
            adp002_R03.SetDatabaseLogon(o_ads007.va_ide_usr, o_ads007.va_pas_usr, o_ads007.va_ser_bda + "\\" + o_ads007.va_ins_bda, o_ads007.va_nom_bda);
            // Paso los datos obtenidos del procedimiento en la anterior ventana
            adp002_R03.SetDataSource(frm_dat);
            // Para enviar parametros directos al reporte (nombre del parametro en crystal report, valor que se enviara)
            adp002_R03.SetParameterValue("vc_nom_emp", va_nom_emp);
            adp002_R03.SetParameterValue("vc_gru_ini", vp_gru_ini);
            adp002_R03.SetParameterValue("vc_gru_fin", vp_gru_fin);
            adp002_R03.SetParameterValue("vc_tip_in1", va_tip_in1);
            adp002_R03.SetParameterValue("vc_tip_fi1", va_tip_fi1);
            adp002_R03.SetParameterValue("vc_atr_in1", vp_atr_in1);
            adp002_R03.SetParameterValue("vc_atr_fi1", vp_atr_fi1);
            adp002_R03.SetParameterValue("vc_tip_in2", va_tip_in2);
            adp002_R03.SetParameterValue("vc_tip_fi2", va_tip_fi2);
            adp002_R03.SetParameterValue("vc_atr_in2", vp_atr_in2);
            adp002_R03.SetParameterValue("vc_atr_fi2", vp_atr_fi2);
            adp002_R03.SetParameterValue("vc_est_ado", vp_est_ado);
            adp002_R03.SetParameterValue("vc_ide_usr", o_ads007.va_ide_usr);
        }
