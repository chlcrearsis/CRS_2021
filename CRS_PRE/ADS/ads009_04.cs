...
namespace CRS_PRE
{
    ...
    /*  Aplicación: ads008 - Permiso Tipo Usuario de Lista de Precio      */
    /* Descripción: Permiso sobre Aplicación                              */
    /*       Autor: JEJR - Crearsis             Fecha: 31-08-2023         */
    /**********************************************************************/
    public partial class ads009_04 : Form
    {
        ...
        private void frm_Load(object sender, EventArgs e)
        {
            ...
            Tabla = new DataTable();
            Tabla = o_ads009.Fe_tus_lis(int.Parse(tb_ide_tus.Text.Trim()));
            for (int i = 0; i < Tabla.Rows.Count ; i++)
            {
                ...
                dg_res_ult.Rows[i].Cells["va_mon_lis"].Value = Tabla.Rows[i]["va_mon_lis"].ToString().Trim();                
        ...
        private void bt_ace_pta_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (msg_res == DialogResult.OK)
                {
                    ...
                    o_ads009.Fe_eli_min(int.Parse(tb_ide_tus.Text.Trim()), "cmr001");
                    // Asigna los permisos que el usuario ha seleccionado
                    for (int i = 0; i < dg_res_ult.RowCount; i++)
                    {
                        bool chk_val = (bool)dg_res_ult.Rows[i].Cells["va_per_mis"].Value;
                        string cod_lis = dg_res_ult.Rows[i].Cells["va_cod_lis"].Value.ToString();

                        if (chk_val == true)
                            o_ads009.Fe_nue_reg(int.Parse(tb_ide_tus.Text.Trim()), "cmr001", cod_lis);
                    }
