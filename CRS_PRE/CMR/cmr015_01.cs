...
namespace CRS_PRE
{
    public partial class cmr015_01 : Form
    {
        ...
        private void fi_bus_car(string ar_tex_bus = "", int ar_prm_bus = 0, string ar_est_bus = "T")
        {
            ...
            tabla = o_cmr015.Fe_bus_car(ar_tex_bus, ar_prm_bus, ar_est_bus);
            ...
            if (tabla.Rows.Count > 0)
            {
                ...
                {
                    dg_res_ult.Rows.Add();
                    dg_res_ult.Rows[i].Cells["va_cod_del"].Value = tabla.Rows[i]["va_cod_del"].ToString();
                    dg_res_ult.Rows[i].Cells["va_nom_del"].Value = tabla.Rows[i]["va_nom_del"].ToString();
                    dg_res_ult.Rows[i].Cells["va_por_del"].Value =tabla.Rows[i]["va_por_del"].ToString();
                ...
                tb_sel_bus.Text = tabla.Rows[0]["va_cod_del"].ToString();
                lb_des_bus.Text = tabla.Rows[0]["va_nom_del"].ToString();
            }
        ...
        private void fi_con_sel()
        {
            ...
            tabla = o_cmr015.Fe_con_del(int.Parse(tb_sel_bus.Text));
            if (tabla.Rows.Count == 0)
            {
                lb_des_bus.Text = "** NO existe";
                return;
            ...
            lb_des_bus.Text = Convert.ToString(tabla.Rows[0]["va_nom_del"].ToString());
        }
        /// <summary>
        /// - > Función que selecciona la fila en el Datagrid que el documento Modificó
        /// </summary>
        private void fi_sel_fil(int cod_del)
        {
            ...
            if (cod_del.ToString() != null)
            {
                try
                {
                    for (int i = 0; i < dg_res_ult.Rows.Count; i++)
                    {
                        if (dg_res_ult.Rows[i].Cells[0].Value.ToString() == cod_del.ToString())
                        {
        ...
        {
            string res_fun = "";
            tab_dat = o_cmr015.Fe_con_del(sel_ecc);
            if (tabla.Rows.Count == 0)
            {
        ...
        /// </summary>
        public void Fe_act_frm(int cod_del)
        {
         if (cb_est_bus.SelectedIndex == 0)
                est_bus = "T";
            if (cb_est_bus.SelectedIndex == 1)
        ...
        private void Mn_delt_doc_Click(object sender, EventArgs e)
        {
